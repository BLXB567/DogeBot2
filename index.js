...
const starts = async (dogexeon = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('Mayumi DVT', {
		font: 'Standard',
	...
	console.log(color('\n> YT CHANNEL: BLZB5 ','silver'))
console.log(color('> GITHUB: BLZXB5 ','silver'))
console.log(color('> WA NUMBER: +6283117436733 ','silver'))
console.log(color('  DVT Bot Inc. 2022','mediumseagreen'))
	dogexeon.browserDescription = [ 'Devil', 'ubuntu', '3.0' ]
...
dogexeon.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "6283117436733@s.whatsapp.net"
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `Mayumi DVT`, img,but = [{buttonId:`getdescgc`, 
			...
			try {
				...
				teks = `*Gᴏᴏᴅ Bʏᴇ* 👋\n@${num.split('@')[0]}\n*𝙷𝚘𝚙𝚎𝚏𝚞𝚕𝚕𝚢 𝚃𝚑𝚎𝚛𝚎 𝚆𝚘𝚗𝚝 𝙱𝚎 𝙱𝚞𝚛𝚍𝚎𝚗 𝙷𝚎𝚛𝚎 𝙰𝚗𝚢𝚖𝚘𝚛𝚎* ❄️`

				sendButImage(anu.jid, teks, `Mayumi DVT`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `GOODBYE !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
...
dogexeon.on('group-update', async (anu) => {
    ...
    dogexeon.on('chat-update', async (message) => {
        ...
        ownerNumber = ["6283117436733@s.whatsapp.net","6283117436733@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
